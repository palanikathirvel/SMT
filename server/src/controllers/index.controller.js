...
module.exports = {
    ...
    editAvatar: async (req, res, next) => {
        try {
            ...
            if (req.user.avatar && req.user.avatar.id) {
                try {
                    await cloudinary.uploader.destroy(req.user.avatar.id);
                } catch (deleteErr) {
                    console.log("Error deleting existing avatar:", deleteErr);
            ...
            if (req.file && req.file.path) {
                try {
                    ...
                    console.log("Error cleaning up temp file:", unlinkErr);
                }
            }
            response.error(res);
        }
    ...
    getStats: async (req, res, next) => {
        try {
            ...
            if (req.user.role === roles.Student) {
                ...
                // For students, show total available mentors instead of mentees
                studentsCount = await Mentor.countDocuments({ isBanned: false });
            }
