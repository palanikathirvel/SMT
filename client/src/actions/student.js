...
export const getAvailableMentors = (setMentors) => async (dispatch) => {
    try {
        console.log('Making API call to get available mentors...');
        const { data } = await api.getAvailableMentors();
        console.log("available mentors response:", data);
        ...
        if (data && data.code === 200) {
            ...
            setMentors([]);
            showToast("error", data?.msg || "Failed to fetch mentors", 10000, toast.POSITION.BOTTOM_LEFT);
        }
    } catch (error) {
        console.error('Network error:', error);
        setMentors([]);
        showToast("error", "Network error while fetching mentors", 10000, toast.POSITION.BOTTOM_LEFT);
    }
};

export const sendMentorRequest = (fields, onSuccess) => async (dispatch) => {
    try {
        const { data } = await api.sendMentorRequest(fields);
        console.log("mentor request sent", data);

        if (data.code === 200) {
            showToast("success", data.msg, 3000, toast.POSITION.TOP_RIGHT);
...
export const getMyMentorRequests = (setRequests) => async (dispatch) => {
    try {
        const { data } = await api.getMyMentorRequests();
        console.log("my mentor requests", data);
