...
module.exports = {
    ...
    getMentorRequests: async (req, res, next) => {
        try {
            const requests = await MentorRequest.find({ 
                mentor: req.user._id, 
                status: "pending" 
            })
            .populate('student', 'firstname lastname email department semester enrollment_no avatar')
            .sort({ createdAt: -1 });
    ...
    acceptMentorRequest: async (req, res, next) => {
        try {
            ...
            if (!mentorRequest) {
                return response.notfound(res, "Request not found", {});
            }

            // Check if student already has a mentor
            const student = await Student.findById(mentorRequest.student._id);
            if (student.mentoredBy) {
                return response.error(res, "Student already has a mentor", {});
            }
            ...
            student.mentoredBy = req.user._id;
            await student.save();

            // Update mentor's student count
            const studentCount = await Student.countDocuments({ mentoredBy: req.user._id });
            req.user.studentCount = studentCount;
            await req.user.save();

            // Reject all other pending requests from this student
            await MentorRequest.updateMany(
                { student: student._id, status: "pending", _id: { $ne: requestId } },
                { status: "rejected" }
            );

            response.success(res, "Request accepted successfully", {});
            next();
        } catch (err) {
            console.log(err);
            response.error(res);
    ...
    rejectMentorRequest: async (req, res, next) => {
        try {
            ...
            response.success(res, "Request rejected successfully", {});
            next();
        } catch (err) {
            console.log(err);
            response.error(res);
