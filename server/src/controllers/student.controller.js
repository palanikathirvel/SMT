...
module.exports = {
    ...
    // send mentor request
    sendMentorRequest: async (req, res, next) => {
        try {
            ...
            if (req.user.mentoredBy) {
                return response.error(res, "You already have a mentor", {});
            }
            ...
            if (existingRequest) {
                return response.error(res, "Request already sent to this mentor", {});
            }
            ...
            response.success(res, "Request sent successfully", {});
            next();
        } catch (err) {
            console.log(err);
            response.error(res);
    ...
    getMyMentorRequests: async (req, res, next) => {
        try {
            const requests = await MentorRequest.find({ student: req.user._id })
                .populate('mentor', 'firstname lastname department designation avatar')
                .sort({ createdAt: -1 });
